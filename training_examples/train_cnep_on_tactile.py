"""
Training example for CNEP with tactile image and force conditioning.

This script demonstrates how to train a CNEP model on robot tactile data,
...
num_decoders = 4  # Number of expert decoders in CNEP
...
optimizer = torch.optim.Adam(lr=3e-4, params=cnep_.parameters())
    ...
    if epoch % val_per_epoch == 0:
        with torch.no_grad():
            ...
            for j in range(v_epoch_iter):
                ...
                pred, gate = cnep.val(val_obs, val_tar_x, val_obs_mask)
                dec_id = torch.argmax(gate.squeeze(1), dim=-1)
                pred_means = pred[dec_id, torch.arange(batch_size), :, :dy]
            ...
            if val_err < min_val_err:
                ...
                torch.save(cnep_.state_dict(), f'{root_folder}saved_models/cnep_best.pt')
            else:
...
print(f"Model saved to: {root_folder}saved_models/cnep_best.pt")
print("="*70)
